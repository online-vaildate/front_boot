...
function getPermission(data, service, type, organizationId, projectId) {
  ...
  if (_.indexOf(approveArray, false) >= 0) {
    return false;
  } else {
    return true;
  }
...
function getPermissionFlag(data, service, type, organizationId, projectId) {
  let permission;
  let permissionArray = [];
  if (service) {
    ...
    axios.post('/iam/v1/permissions/checkPermission', JSON.stringify(permissionArray)).then((data) => {
      AppState.setPerMission(data);
    }).then(() => {
      return this.getPermission(AppState.getPerMission, service, type, organizationId, projectId);
    })
  }
